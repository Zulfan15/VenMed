...
export default function PrescriptionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getStatusBadge = (status: string) => {
    const config: Record<string, { color: string; text: string }> = {
      issued: { color: 'bg-blue-100 text-blue-900', text: 'Terbit' },
      dispensed: { color: 'bg-green-100 text-green-800', text: 'Sudah Diambil' },
      expired: { color: 'bg-slate-100 text-slate-600', text: 'Kadaluarsa' },
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="mb-6 flex items-center gap-4">
          <Link href="/prescription/history">
            <button type="button" aria-label="Kembali ke Riwayat" title="Kembali ke Riwayat" className="p-2 hover:bg-slate-200 rounded-lg text-slate-600">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-800">{prescription.doctor.name}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-600">SIP:</span>
              <span className="font-medium text-slate-800">{prescription.doctor.license_number}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-slate-800">Resep Diterbitkan</p>
                <p className="text-sm text-slate-600">{formatDate(prescription.issued_at)}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-slate-800">Berlaku Sampai</p>
                <p className="text-sm text-slate-600">{formatDate(prescription.expires_at)}</p>
              </div>
            </div>
            ...
            {prescription.dispensed_at && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-800">Obat Diambil</p>
                  <p className="text-sm text-slate-600">{formatDate(prescription.dispensed_at)}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="mt-6 flex gap-3">
          <Link href="/prescription/history" className="flex-1">
            <button className="w-full px-6 py-3 bg-slate-600 text-white rounded-lg font-semibold hover:bg-slate-700 transition-colors">
              Kembali
            </button>
          </Link>
          {prescription.status === 'issued' && (
            <Link href={`/prescription/qr/${prescription.id}`} className="flex-1">
              <button className="w-full px-6 py-3 bg-blue-900 text-white rounded-lg font-semibold hover:bg-blue-800 transition-colors">
                Lihat QR Code
              </button>
            </Link>
          )}
